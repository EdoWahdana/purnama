	...
	<div class="container-fluid px-0">
		...
		<div class="konten">
			<div class="rounded-top bg-white shadow p-5 mx-auto my-5" style="width: 75%; border-bottom: 5px solid teal;">
				...
				<p>
					Alamat: <span id="alamat"><?php echo "$alamatLengkap"; ?></span><br><button class="tombol tombol-teal mt-2" id="ubah">Ubah Alamat</button><br>
				...
				<div class="text-right mt-5">
					...
					<button type="button" class="tombol tombol-teal" id="submitTukar">Proses Penukaran Poin</button>
				</div>
			</div>
		</div>
...
<script>
	...
	$("#submitTukar").on("click", function() {
		...
		$(".helper span").html("");
		updateTransaksiPoin("status", "proses");
	});
	...
	$("#ongkir").on("change keyup", function() {
		$.ajax({
			...
			success: function (respon) {
				...
				updateTransaksiPoin("idOngkir", $("#ongkir").val());
			}
		});
	});
		...
		$.ajax({
			...
			success: function (respon) {
				...
				if (respon.status == "Proses") {
					updateTransaksiPoin("alamat", $("#alamatLengkap").val());
					updateTransaksiPoin("desa", $("#desa").val());
					updateTransaksiPoin("kecamatan", $("#kecamatan").val());
					updateTransaksiPoin("kabupaten", $("#kabupaten").val());
					updateTransaksiPoin("provinsi", $("#provinsi").val());
					updateTransaksiPoin("tanggal", "<?php echo date("Y-m-d"); ?>");
					updateTransaksiPoin("jam", "<?php echo date("h:i:s"); ?>");
					location.href = "detail_pembayaran_poin.php?id=<?php echo $id; ?>";
