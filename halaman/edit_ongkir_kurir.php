...
$query = mysqli_query($conn, "SELECT * FROM ongkirkurir WHERE idOK='$id'");
...
<html lang="en">
<head>
  <title>Edit Paket Pengiriman</title>
  <meta charset="utf-8">
...
<body>
	<div class="container-fluid px-0">
		...
		<div class="konten container mt-5">
			<a href="data_ongkir_kurir.php" class="tombol tombol-teal mb-3"><i class="fas fa-chevron-circle-left"></i> Kembali</a>
			<div class="row">
					...
					<div class="bg-white shadow-sm p-4 mb-5" style="border-top: 3px solid teal;">
						<form enctype="application/x-www-form-urlencoded" id="formIsian">
							...
							<div class="row mb-3">
								...
								<div class="col-lg-9">
									<input type="text" class="form-control" id="nama" name="nama" value="<?php echo $namaOngkir; ?>">
								</div>
							</div>
...
<script>
	...
	$("#formIsian").on("submit", function(e) {
		...
		formData.append("idKurir", "<?php echo $idKurir; ?>");
		$("#submitIsian").prop("disabled", true);
		$(".helper div").prop("hidden", false);
		$(".helper span").html("");
		$.ajax({
			url: "../include/updateOngkirKurir.php",
			type: "POST",
			data: formData,
			contentType: false,
			cache: false,
			...
			success: function (respon) {
				...
				if (respon.status == "Berhasil") {
					location.href = "data_ongkir_kurir.php?id=<?php echo $idKurir; ?>";
				} else {
