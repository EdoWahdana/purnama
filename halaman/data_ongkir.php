			...
			<div class="row">
				...
				<div class="col-lg-9 col-8">
					<div class="bg-white shadow-sm p-4 mb-5" style="border-top: 3px solid teal;">
						<h4>Harga Ongkos Kirim <?php echo $namaKurir; ?></h4>
						<div class="row mb-3">
							<?php
							...
							if (mysqli_num_rows($query) > 0) {
								while ($data = mysqli_fetch_assoc($query)) {
									...
									<a href='data_ongkir.php?id=$data[idKurir]' class='tombol tombol-teal my-3'>$data[namaKurir]</a>
									</div>";
								}
							} else
						...
						<a href="tambah_ongkir.php?id=<?php echo $id; ?>" class="tombol tombol-teal my-3">Tambah Ongkos Kirim</a>
						<div class="table-responsive-lg">
							<table class="table table-sm">
								...
								<tbody>
									<?php
									$query = mysqli_query($conn, "SELECT * FROM ongkir INNER JOIN ongkirkurir ON ongkir.idOK=ongkirkurir.idOK WHERE ongkir.idKurir='$id'");
									if (mysqli_num_rows($query) > 0) {
										$no = 1;
										while ($data = mysqli_fetch_assoc($query)) {
											echo "<tr>
											...
											<td>$data[prov]</td>
											<td>
												<a class='tombol tombol-teal mr-3' href='edit_ongkir.php?id=$data[idOngkir]'><i class='fas fa-edit'></i> Edit</a> 
												<button class='tombol tombol-red' name='hapus[]' id='$data[idOngkir]'><i class='fas fa-trash-alt'></i> Hapus</button>
											</td>
											</tr>";
											$no++;
										}
...
<script>
	$("button[name='hapus[]']").on("click", function() {
		...
		$.ajax({
			url: "../include/deleteOngkir.php",
			type: "POST",
			data: {id: $(this).prop("id")},
			dataType: "JSON",
			success: function (respon) {
				if (respon.status == "Berhasil") {
					location.href = "data_ongkir.php";
				} else {
